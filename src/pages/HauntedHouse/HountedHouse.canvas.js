...
export default function draw(canvas) {
  ...
  const graveMaterial = new THREE.MeshStandardMaterial({ color: "#beb6b1" });
  ...
  const ghost2 = new THREE.PointLight("#0056ff", 2, 3);
  scene.add(ghost2);
  ...
  const adjustShadowMapSize = (object, option) => {
    const { width, height, far } = option;

    object.shadow.mapSize.width = width;
    object.shadow.mapSize.height = height;
    object.shadow.camera.ar = far;
  };

  adjustShadowMapSize(moonLight, {
    width: 256,
    height: 256,
    far: 15,
  });
  [doorLight, ghost1, ghost2, ghost3].forEach((object) => {
    adjustShadowMapSize(object, {
      width: 256,
      height: 256,
      far: 7,
    });
